...
function InvoicesPage() {
  ...
  const handleExport = () => {
    ...
    if (!exportInvoicesToExcel(toExport)) {
...
function Row({ inv }: { inv: Invoice }) {
  ...
  return (
    <Link
      to="/invoices/$id"
      params={{ id: inv.id }}
      className={`${TABLE_GRID} px-4 sm:px-6 py-4 hover:bg-muted/40 transition`}
    >
      ...
      <p className="hidden md:block text-sm text-muted-foreground">{formatInvoiceListDate(inv.issueDate)}</p>
