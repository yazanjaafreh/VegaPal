...
import {
  ...
  Sparkles, Banknote, RefreshCw, Loader2, ArrowLeftRight, ChevronDown, Menu, Mail, Headphones,
  Copy, AlertTriangle,
} from "lucide-react";
import {
  type CurrencyCode,
...
function LiveCurrencyConverter() {
  ...
  const handleSwap = () => {
    ...
    const nextAmount =
      Number.isFinite(converted) && !ratesUnavailable
        ? String(converted)
        : amount;
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6">
        ...
        <div className="rounded-[20px] bg-white p-4 sm:p-6 lg:p-10 shadow-[0_24px_80px_rgba(15,23,42,0.12)] min-w-0 overflow-hidden">
          <div className="flex flex-col gap-8 md:grid md:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] md:items-start md:gap-5 lg:gap-8">
            ...
            <div className="space-y-4">
              <p className="text-[11px] font-bold tracking-[0.18em] text-muted-foreground">{t("converter.to")}</p>
              <output
              ...
              >
                {loading ? (
                  ...
                  <span key={`${from}-${to}-${convertedDisplay}`} className="animate-in fade-in duration-300">
                    {convertedDisplay}
                  </span>
                )}
              </output>
...
function PaymentQr({ value, className }: { value: string; className?: string }) {
  ...
  useEffect(() => {
    QRCode.toDataURL(value, { margin: 1, width: 168 }).then(setSrc).catch(() => setSrc(null));
  }, [value]);
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Create professional USDT invoices and accept crypto payments. Built for freelancers and businesses who want secure, trusted deals." },
      { property: "og:title", content: "VegaPal — Secure USDT Invoices" },
      { property: "og:description", content: "Professional crypto invoicing for freelancers and businesses." },
    ],
  }),
  component: Landing,
});
